#!/bin/ksh
#
# Program Name	: trx-bas-accum_lst.sh
# Description	: Creates log listing of TRRX-BAS Accumulator file.
...
usage: trx-bas-accum_lst.sh [-d <ccyymmdd>]
...
set_filename()
{
        PGP_FILE="BAS-ACCUM-${INPUT_DATE}.txt.pgp"
	ELIG_FILE="BAS-ACCUM-${INPUT_DATE}.txt"
}
...
create_listing()
{
	LOG_NAME=${CLIENT}l-${DATE}.log
        cd ${ELIG_DIR}
        echo "TRRX-BAS Accumulator File" > ${ELIG_LOG}/${LOG_NAME}
