...
#		: 12/04/2009 (LSJ) Cleaned up find commands and added scp ALL files to both prod10 and prod11
#		: 12/06/2009 (LSJ) Added CHECKRB001 extract
...
then
  ...
  susp_extract
  echo 
  echo "--> Starting check file extract - check04"
  check_extract
fi
...
then
	...
	then
		cd ${DIR_2}
		find . -type f ! -name "CL72.*" ! -name "*-files.zip" -mtime 0 -print | zip ${DATE}-files.zip -@
		scp -q ${DATE}-files.zip prod11:/usr/pdm/sqlimports/misc
		ssh prod11 "unzip -od /usr/pdm/sqlimports/misc /usr/pdm/sqlimports/misc/${DATE}-files.zip"
		scp -q ${DATE}-files.zip prod10:/usr/pdm/sqlimports/misc
		ssh prod10 "unzip -od /usr/pdm/sqlimports/misc /usr/pdm/sqlimports/misc/${DATE}-files.zip"
	fi
fi
...
then
        cd ${DIR_2}
        scp -q CL72.${DATE}.sql* prod10:/usr/pdm/sqlimports/claims
        scp -q wh_daily_claims_count prod10:/usr/pdm/sqlimports/claims
        scp -q CL72.${DATE}.sql* prod11:/usr/pdm/sqlimports/claims
        scp -q wh_daily_claims_count prod11:/usr/pdm/sqlimports/claims
fi
