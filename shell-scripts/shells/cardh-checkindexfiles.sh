#!/bin/sh
#
# Program Name	: cardh-checkindexfiles.sh
# Description	:
...
DATE=`date +%Y%m%d`
OUT_LOG=/tmp/cardh-checkindexfiles_${USER}_${DATE}.log
RECLOGDIR=/tmp
...
usage: cardh-checkindexfiles.sh 
...
VERIFYFLOG=/tmp/compu13log_cardh_${USER}_${DATE}.txt; export VERIFYFLOG
INFILE=/usr/lnk/log/PARMFILE-COMPU13-CARDH.txt

date > ${OUT_LOG}
   ${SHELL_DIR}/compu13.sh -i ${INFILE} -o $VERIFYFLOG >> ${OUT_LOG} 2>&1
   COMPU13_RETVAL=$?
