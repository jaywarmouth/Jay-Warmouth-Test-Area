#!/bin/ksh
#
# Program Name	: trx-sis-accum_lst.sh
# Description	: Creates log listing of TRRX-Sisco Accumulator file.
...
usage: trx-sis-accum_lst.sh [-d <ccyymmddhhmm>]
...
set_filename()
{
        PGP_FILE="SiscoTRX_accum_${INPUT_DATE}.dat.pgp"
	ELIG_FILE="SiscoTRX_accum_${INPUT_DATE}.dat"
}
...
create_listing()
{
	LOG_NAME=${CLIENT}l-${DATE}.log
        cd ${ELIG_DIR}
        echo "TRRX-Sisco Accumulator File" > ${ELIG_LOG}/${LOG_NAME}
