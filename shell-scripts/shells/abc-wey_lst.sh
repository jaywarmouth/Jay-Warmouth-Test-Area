#!/bin/ksh
#
# Program Name	: abc-wey_lst.sh
# Description	: Creates log listing of ABC/Weyco (Industrial Steel) eligibility file.
...
usage: abc-wey_lst.sh [-d <ccyymmdd>]
...
set_filename()
{
        PGP_FILE="F1OB834_${INPUT_DATE}??????.txt.pgp"
	ELIG_FILE="F1OB834${INPUT_DATE}??????.x12"
}
...
move_files()
{
	...
	mv ${ELIG_DIR}/${ELIG_FILE} ${ELIG_DIR}/ise${DATE}-X12
	cp ${ELIG_DIR}/ise${DATE}-X12 ${ELIG_ARCH}
}
...
create_listing()
{
	LOG_NAME=abc-wey${DATE}.log
        cd ${ELIG_DIR}
        echo "ABC/Industrial Steel(spo0450) Eligibility File" > ${ELIG_LOG}/${LOG_NAME}
        echo "---------------------------------------------------" >> ${ELIG_LOG}/${LOG_NAME}
        echo "" >> ${ELIG_LOG}/${LOG_NAME}
        ls -l ise${DATE}-X12 >> ${ELIG_LOG}/${LOG_NAME}
