#!/bin/sh
#
# Program Name	: wh_prod_dly.sh
# Description	: Procedure to create extract files for Warehouses
#		  Command Line Arguments:
#		  -u <run type>
# Author	: Linda S. Jefferis
# Date		: 06/19/2016
# Modifications	: 08/18/2016 - Add carange01.sh process TT16007-3
#		: 09/14/2016 - add excep_extract to stop issues with extract on prod11.
#		: 05/09/2017 - TT13915-50; add claim80rb and cardirb001 procedures.
#		: 06/26/2017 - TT17259-4; add onetm001 procedure.
#		: 09/09/2017 - Change cardh52 to full.
#		: 11/28/2017 - TT:17539-1 (Add dr340brb01)
#		: 09/18/2018 - TT13915-65 (add rever04)
#		: 04/30/2020 - TT3200-301 (numotrb01)
#		: 10/19/2021 - MULTOV project (multovrb01)
#		: 12/28/2021 - Moved CLMSG from wh_daily_files.sh to here.
#		: 01/17/2022 - Moved SUSPRB001 and NPICMSRB001 processes from wh_daily_files.sh
#		: 12/21/2022 - Add REJMSGRB001 (rejmsg01) logic and move REJECRB001 (reject02) from prod11 process.
#		: 01/05/2023 - Move of override (override01) exctract from prod11 processes.
#		: 07/18/2023 - Move of gentb (gentb02) extract from prod11 processes.
#		: 08/15/2023 - Add mconfrb01 extract process
#		: 07/08/2024 - Add DRGPRCRB001 extract process
#		: 04/01/2025 - convert_to_batch.sh error trapping
#               : 05/11/2026 - add AWS bucket to copy the data warehouse file to aws location
#               : 05/20/2026 - DR340B0MAS file decommission. - remove from DW extract
...
DR340B_START_DATE=`date -d "-3 days 0800" +%Y%m%d`
SYS="00000000"
...
{
        echo
        echo "--> onetm extract - onetm001"
        echo "Process Date Range: ${PROCESS_DATE}${PROCESS_DATE}"
        ${SHELL_DIR}/onetm001.sh -d ${PROCESS_DATE}${PROCESS_DATE} > ${RPT_DIR}/onetm001 2>&1
        echo "     `grep "WRITTEN" ${RPT_DIR}/onetm001`"
        echo "     `grep "ERROR" ${RPT_DIR}/onetm001`"
        FNAME=${ONETMRB001}
        file_transfer
}
...
numot_extract()
{
        ...
        ${SHELL_DIR}/numotrb01.sh -b yesterday  > ${RPT_DIR}/numotrb01 2>&1
...
dr340b_extract()
{
        ...
        echo "Process Date Range: ${DR340B_START_DATE}${PROCESS_DATE}"
        ${SHELL_DIR}/dr340brb01.sh -d ${DR340B_START_DATE}${PROCESS_DATE} > ${RPT_DIR}/dr340brb01 2>&1
...
phdem_extract()
{
        ...
        ${SHELL_DIR}/phdem03.sh -f > ${RPT_DIR}/phdem03 2>&1
...
phnet_extract()
{
        ...
        NET=000000999999
        ${SHELL_DIR}/phnet12.sh -a ${NET} -f > ${RPT_DIR}/phnet12 2>&1
        echo "     TOTAL RECORDS:  `wc -l ${PHNETRB001}`"
        echo "     `grep -a "ERROR" ${RPT_DIR}/phnet12`"
        FNAME=${PHNETRB001}
        file_transfer
}
