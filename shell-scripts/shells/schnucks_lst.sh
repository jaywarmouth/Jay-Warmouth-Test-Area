#!/bin/ksh
#
# Program Name	: schnucks_lst.sh
# Description	: Creates log listing of Schnucks eligibility file.
...
usage: schnucks_lst.sh -d <ccyymmdd>
...
set_filename()
{
        PGP_FILE="schnucks_${INPUT_DATE}.pgp"
	ELIG_FILE="schnucks_${INPUT_DATE}"
}
...
create_listing()
{
	LOG_NAME=${CLIENT}e-${DATE}.log
        cd ${ELIG_DIR}
        echo "Schnucks (spo0606) Eligibility File" > ${ELIG_LOG}/${LOG_NAME}
