#!/bin/ksh
#
# Program Name	: appro-cyp-accum_lst.sh
# Description	: Creates log listing of ApproRx accumulator file sent from Cypress Benefit.
...
REMOTE_DIR="/usr/lnk/wt/cyb-ftps/ToPDMI"
...
usage: appro-cyp-accum_lst.sh [-d <ccyymmdd>]
...
set_filename()
{
	ACCUM_FILE="cyp_appro_accum_${INPUT_DATE}.txt"
}
...
create_listing()
{
	LOG_NAME=${CLIENT}l-${DATE}.log
        cd ${ELIG_DIR}
        echo "ApproRx-C&K MKTS (1193) Accumulator File" > ${ELIG_LOG}/${LOG_NAME}
