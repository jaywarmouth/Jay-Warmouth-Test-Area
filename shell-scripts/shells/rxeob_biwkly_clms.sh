#!/bin/sh
#
# Program Name	: rxeob_biwkly_clms.sh
...
TAPE_FILE=???CL111RX-P-RXEOB			# Original file
...
usage: rxeob_biwkly_clms.sh [-d <mmdd>]
	mmdd - current date
...
set_filenames()
{
   ...
   then
      ...
      ZIP_FILE=${FILE_DATE}.zip      # Zip file name
      CLM_FILE=biwk_${FILE_DATE}.txt	# New File name
   fi
}
