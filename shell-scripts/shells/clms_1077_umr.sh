#!/bin/sh
#
# Program Name	: clms_1077_umr.sh
# Description	: Procedure to setup claims file for TRRX/Centaur and send to UMR
...
usage: clms_1077_umr.sh -p <p/e date>
...
set_filenames()
{
	CLM_FILE="MDSD53_P_Centaur_rxclaims_${PE_DATE}"
}
...
if test $? -eq 0
then
	cat ${FILE_LOC}/${TEXT_FILE} | ${MAIL_PROG} -s "Centaur Rx Claims File Notification" -c ${MAIL_CC} ${MAIL_TO}
else
