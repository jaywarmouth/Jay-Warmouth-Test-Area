#!/bin/ksh
#
# Program Name	: trx-php-accum_lst.sh
# Description	: Creates log listing of TRRX-PHP Accumulator file.
#                 Command Line Arguments:
#                 -d <ccyymmddhhmm> - date on file sent.
...
usage: trx-php-accum_lst.sh [-d <ccyymmddhhmm>]
...
set_filename()
{
        PGP_FILE="PHP_PDMI_AC${INPUT_DATE}.txt.pgp"
	ELIG_FILE="PHP_PDMI_AC${INPUT_DATE}.txt"
}
...
create_listing()
{
	LOG_NAME=${CLIENT}l-${DATE}.log
        cd ${ELIG_DIR}
        echo "TRRX-PHP Accumulator File" > ${ELIG_LOG}/${LOG_NAME}
