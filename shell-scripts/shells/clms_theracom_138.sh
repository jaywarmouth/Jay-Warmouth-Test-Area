#!/bin/ksh
#
# Program Name	: clms_theracom_138.sh
# Description	: Procedure to provide cycle refresh file for Lash-Vidara (sys0138)
...
TMP_LOC="/tmp"
TAPE_FILE="???CL111D0-P-LVID"
TEXT_FILE="???CL111D0-P-LVIDTEXT"
...
usage: clms_theracom_138.sh -p <p/e date>
...
set_filenames()
{
	...
	CLM_FILE="bimonthlyTHCVidara-clms${FILE_DATE}.txt"
	LOG_FILE="bimonthlyTHCVidara-totals${FILE_DATE}.txt"
}
...
ftp_file()
{
	...
	then
	   ...
	   echo "The files for P/E ${PE_DATE} are now available." | ${MAIL_PROG} -s "TheraCom-Vidara Cycle Refresh File Notification" -c ${MAIL_CC} ${MAIL_TO}
	else
