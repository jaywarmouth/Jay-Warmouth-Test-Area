...
ACC_OUT=/usr/lnk/elig_out
FG4AUD_DIR="/usr/lnk/audit"
AUDNAME="LIMAUD"
DATE="null"
CLIENT="null"
OUT_FULL_FILE_DIR="/usr/lnk/accum_out"
SHELL="/usr/lnk/shell"
UPDATE_FILE=0
TEST_MODE=0
FULL_TRANSFER=0
...
set_variables()
{
   ...
   if [ ${FULL_TRANSFER} = 1 ]
   then
	ACCUM01TAP=${OUT_FULL_FILE_DIR}/${CLIENT_NAME}-LIMIT-${FULL_DATE}
	GROUP00TMP=/tmp/GROUP00TMP; export GROUP00TMP
...
# Move output file from full file run
archive_full_file()
{
	if test -s ${ACCUM01TAP}
	then
		cp ${ACCUM01TAP} ${ACC_OUT}/sys${SYS}
	else
		echo "-*> The ACCUM01TAP file does not exist. Check for processing errors."
		ls -l ${ACCUM01TAP}
		rm -f ${ACCUM01TAP} ${SPONS00TMP} ${GROUP00TMP}
		exit 99
	fi
}
	...
	then
		...
		then
			...
			if test $? -eq 0
				...
				if [ $EMAILTO != "NA" ]
				then
					${SHELL}/accum01_summary.sh -c ${CLIENT} -f ${ACCUMERR_RPT}
				fi
			fi
		fi
	fi
...
cleanup ()
{
   ...
   mv ${ACC_OUT}/${CLIENT}l${DATE} ${ACC_OUT}/sys${SYS}
   rm -f ${ACCUM01TAP}
}
