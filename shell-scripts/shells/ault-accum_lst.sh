#!/bin/sh
#
# Program Name	: ault-accum_lst.sh
# Description	: Moves files and creates log listing of Aultcare accum file.
...
usage: ault-accum_lst.sh [-d <ccyymmdd>]
...
convert_date()
{
        DATE=`echo ${INPUT_DATE} | cut -c5-8`-2nd
}
...
set_filename()
{
	ACCUM_FILE="AULT_accum_${INPUT_DATE}.txt"
}
...
create_listing()
{
        ...
        cd ${ELIG_DIR}
        echo "Aultcare 2nd Accumulator File" > ${ELIG_LOG}/${LOG_NAME}
