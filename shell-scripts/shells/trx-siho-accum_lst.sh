#!/bin/ksh
#
# Program Name	: trx-siho-accum_lst.sh
# Description	: Creates log listing of TRRX-SIHO Accumulator file.
#                 Command Line Arguments:
#                 -d <ccyymmdd> - date on file sent.
# Author	: Linda S. Jefferis
# Date		: 02/25/2013
# Modifications : 06/30/2014 - add coding and variables to run decrypt_file.sh (TT:10901-1)(DME)
#               : 01/12/2015 - add ToPDMI to ftp directory and change File name to new naming. (DME)
#
#
# Variables Used:
...
usage: trx-siho-accum_lst.sh [-d <ccyymmdd>]
...
set_filename()
{
        PGP_FILE="TRXSIHO_accum_${INPUT_DATE}.txt.pgp"
	ELIG_FILE="TRXSIHO_accum_${INPUT_DATE}.txt"
}
...
create_listing()
{
	LOG_NAME=${CLIENT}l-${DATE}.log
        cd ${ELIG_DIR}
        echo "TRRX-SIHO Accumulator File" > ${ELIG_LOG}/${LOG_NAME}
