#!/bin/ksh
#
# Program Name	: abc-mckee-accum_lst.sh
# Description	: Creates log listing of TRRX-PHP Accumulator file.
#                 Command Line Arguments:
#                 -d <ccyymmdd>_<hhmmss> - date/time on file sent.
...
REMOTE_DIR="/usr/lnk/wt/oper-wt/mckee/ToPDMI"
...
usage: abc-mckee-accum_lst.sh [-d <ccyymmdd>]
...
set_filename()
{
	ACCUM_FILE="PDMI_BCBST_MED_ACCMS${INPUT_DATE}.txt"
}
...
create_listing()
{
	LOG_NAME=${CLIENT}l-${DATE}.log
        cd ${ELIG_DIR}
        echo "ABC-McKee (1041) Accumulator File" > ${ELIG_LOG}/${LOG_NAME}
