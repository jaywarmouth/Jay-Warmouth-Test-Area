#!/bin/ksh
#
# Program Name	: ahfmedd_lst.sh
# Description	: Creates log listing and moves AHF-MEDD eligibilityand other files.
#		  Command line arguments:
#		  -d <ccyymmdd> - date for files
# Author	: Linda S. Jefferis
# Date		: 12/14/2014
# Modifications : 12/16/2014 - Change 300 record size to 302; the MEDD elig file is line sequential.
#
# Variables Used:
...
usage: ahfmedd_lst.sh [-d <ccyymmdd>] 
...
set_filename()
{
	ZIP_FILE="ahfmedd_${INPUT_DATE}.zip"
        ELIG_FILE="ahfmedd_elig_${INPUT_DATE}.txt"
	GRP_FILE="ahfmedd_grp_${INPUT_DATE}.txt"
	ACCUM_FILE="ahfmedd_accum_${INPUT_DATE}.txt"
	OTHPAY_FILE="ahfmedd_othpay_${INPUT_DATE}.txt"
}
...
move_files()
{
	...
	if ! test -a ${ELIG_DIR}/${OTHPAY_FILE}
	then
	  echo "-*> Incorrect othpay filename... exiting process"
	  exit 1
	fi
	mv ${ELIG_DIR}/${ELIG_FILE} ${ELIG_DIR}/${CLIENT}e${DATE}
	mv ${ELIG_DIR}/${GRP_FILE} ${ELIG_DIR}/${CLIENT}g${DATE}
	rm ${ELIG_DIR}/${ACCUM_FILE}
	mv ${ELIG_DIR}/${OTHPAY_FILE} ${ELIG_DIR}/${CLIENT}o${DATE}
...
create_listing()
{
        ...
        cd ${ELIG_DIR}
        echo "Aultcare-MEDD Eligibility Files" > ${ELIG_LOG}/${LOG_NAME}
