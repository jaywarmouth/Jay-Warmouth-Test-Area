...
# Description	: Creates log listing of Ennis/GPA (sys76/spo0385) eligibility file.
...
usage: ennis-gpa_lst-wey_lst.sh [-d <mmddccyy>]
...
set_filename()
{
        PGP_FILE="gpa.${INPUT_DATE}.txt.pgp"
	ELIG_FILE="gpa.${INPUT_DATE}.txt"
}
...
move_files()
{
	...
	mv ${ELIG_DIR}/${ELIG_FILE} ${ELIG_DIR}/gpe${DATE}
	cp ${ELIG_DIR}/gpe${DATE} ${ELIG_ARCH}
}
...
create_listing()
{
	LOG_NAME=ennis-gpa${DATE}.log
        cd ${ELIG_DIR}
        echo "Ennis/GPA (spo0385) Eligibility File" > ${ELIG_LOG}/${LOG_NAME}
        echo "---------------------------------------------------" >> ${ELIG_LOG}/${LOG_NAME}
        echo "" >> ${ELIG_LOG}/${LOG_NAME}
        ls -l gpe${DATE} >> ${ELIG_LOG}/${LOG_NAME}
