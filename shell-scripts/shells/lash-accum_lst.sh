#!/bin/ksh
#
# Program Name	: lash-accum_lst.sh
# Description	: Creates log listing of Lash Accumulator file.
#                 Command Line Arguments:
#                 -d <mmddccyy> - date on file sent.
# Author	: Linda S. Jefferis
# Date		: 04/26/2006
# Modifications : 07/01/2014 - add lgoic and variables to run decrypt_file.sh (TT:10901-1)(DME)
#
# Variables Used:
...
FTP_DIR="/usr/lnk/wt/lashp-ftp/topdm"
...
usage: lash-accum_lst.sh [-d <mmddccyy>]
...
set_filename()
{
        PGP_FILE="PANF_PDMI_Accumulator_${INPUT_DATE}??????.txt.pgp"
	ELIG_FILE="PANF_PDMI_Accumulator_${INPUT_DATE}??????.txt"
}
...
move_files()
{
	...
	mv ${ELIG_DIR}/${ELIG_FILE} ${ELIG_DIR}/pfl${DATE}
	cp ${ELIG_DIR}/pfl${DATE} ${ELIG_ARCH}
}
...
create_listing()
{
	LOG_NAME=lash-accum${DATE}.log
        cd ${ELIG_DIR}
        echo "Lash Accumulator File" > ${ELIG_LOG}/${LOG_NAME}
        echo "------------------------------------" >> ${ELIG_LOG}/${LOG_NAME}
        echo "" >> ${ELIG_LOG}/${LOG_NAME}
        ls -l pfl${DATE} >> ${ELIG_LOG}/${LOG_NAME}
