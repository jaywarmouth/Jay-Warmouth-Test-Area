#!/bin/sh
#
# Program Name	: trx-pbaelig_lst.sh
# Description	: Prepares eligibility file sent for TrueRx-PBA 
#                 Command Line Arguments:
#                 -d <ccyymmdd> - date on file downloaded.
# Author	: Linda S. Jefferis
# Date		: 11/23/2015
# Modifications : 12/08/2015 - Remove "_PDMI" from file namings. (DME)
#
# Variables Used:
...
FTP_DIR="/usr/lnk/wt/prba-ftp/TRX/ToPDMI"
...
usage: trx-pbaelig_lst.sh -d <ccyymmdd>
...
set_filenames()
{
	ELIG_FILE="TRXPBA_elig_${INPUT_DATE}.txt"
	PGP_FILE="TRXPBA_elig_${INPUT_DATE}.txt.pgp"
}
...
move_files()
{
        ...
        mv ${ELIG_DIR}/${ELIG_FILE} ${ELIG_DIR}/${CLIENT_ID}e${DATE}
        cp ${ELIG_DIR}/${CLIENT_ID}e${DATE} ${ELIG_ARCH}
}
...
create_listing()
{
        ...
        cd ${ELIG_DIR}
        echo "TRX-PBA(sys0130) Eligibility File" > ${ELIG_LOG}/${LOG_NAME}
        echo "-------------------------------------------------" >> ${ELIG_LOG}/${LOG_NAME}
        echo "" >> ${ELIG_LOG}/${LOG_NAME}
        ls -l ${CLIENT_ID}e${DATE} >> ${ELIG_LOG}/${LOG_NAME}
