#!/bin/sh
#
# Program Name	: tsc-pekinaccum_lst.sh
# Description	: Creates log listing of TrueScripts-Pekin accum file.
...
set_filename()
{
        PGP_FILE="TRXPekin_accum_${INPUT_DATE}.txt.pgp"
	ACCUM_FILE="TRXPekin_accum_${INPUT_DATE}.txt"
}
...
create_listing()
{
	LOG_NAME=${CLIENT}l-${DATE}.log
        cd ${ELIG_DIR}
        echo "TrueRx-Pekin Accumulator File" > ${ELIG_LOG}/${LOG_NAME}
