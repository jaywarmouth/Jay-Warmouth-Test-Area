#!/bin/ksh
#
# Program Name	: pay4.sh
# Description	: Pay-Cycle Report and Update 
#		  Command Line Arguments:
#		  -p <prefix> - p/e prefix (e.g. J15)
# Author	: Linda S. Jefferis
# Date		: 12/09/2004
# Modifications : 10/26/2005 - Changes for Linux  (LSJ)
#		: 12/19/2005 - Fixed path of /fs12  (LSJ)
#		: 01/17/2006 - Removed lp of rpt file  (LSJ)
#		: 10/25/2006 - Removed char_repl logic  (LSJ)
#		: 05/04/2007 - Logic changes for BAL_RPT as per request from warehouse  (LSJ)
...
SQL_FILE="BIWKCLMS"
TST_SQL_FILE="BIWKCLMS-TST"
MAIL_PROG="/bin/mail"
TMP_LOG="/tmp/claim72pdm-info"
BAL_RPT="???CL16-SYS-INV-P"
...
echo ""
${SHELL_DIR}/claim72pdm.sh -c pay > ${RPT_DIR}/pay-claim72pdm 2>&1
#${SHELL_DIR}/claim72pdmtst.sh -c pay > ${RPT_DIR}/pay-claim72pdmtst 2>&1
...
echo ""
mv ${FILE_PATH}/${PREFIX}CL72-P-PDM ${SQL_DIR}/${SQL_FILE}
#mv ${FILE_PATH}/${PREFIX}CL72TST-P-PDM ${SQL_DIR}/${TST_SQL_FILE}
...
then
	echo "The pay-cycle claims refresh file, BIWKCLMS, is now available." | ${MAIL_PROG} -s "PAY_CYCLE CLAIMS REFRESH" ${MAIL_WHSE}
else
	echo "No extract file created. Look for possible issue." | ${MAIL_PROG} -s "PAY_CYCLE CLAIMS REFRESH" ${MAIL_OPER}
fi
