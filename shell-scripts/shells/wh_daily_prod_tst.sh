...
# Date		: 01/09/2010
# Modifications : 03/??/2010 - Changes for eliminating .done files, adding date suffix on output files, and doing gzip for all files.  (LSJ)
#		: 01/07/2011 - Added missing site file process
...
REIMB_EXTRACT="${WH_DIR}/${SQL_MISC}/ohrmbrb001"
...
claim_extract()
{
	...
	${SHELL_DIR}/claim72pdm.sh -c day > ${RPT_DIR}/tst-claim72pdmd 2>&1
	echo "     `grep "WRITTEN" ${RPT_DIR}/tst-claim72pdmd`"
	echo "`grep "ERROR CLAIM00TAP: 9701" ${RPT_DIR}/tst-claim72pdmd`"
	mv ${DIR_1}/???${FILE1} ${WH_DIR}/${SQL_CLMS}/${FILE2}-${DATE}
	REC_CNT=`wc -l ${WH_DIR}/${SQL_CLMS}/${FILE2}-${DATE} | awk '{ print $1 }'`
	echo ${REC_CNT}","${PROCESS_DATE} > ${WH_DIR}/${SQL_CLMS}/wh_daily_claims_count-${DATE}
	OUT_DIR=${SQL_CLMS}
	FNAME=${WH_DIR}/${SQL_CLMS}/wh_daily_claims_count-${DATE}
	file_transfer
	FNAME=${WH_DIR}/${SQL_CLMS}/${FILE2}-${DATE}
	file_transfer
...
card_extract()
{
	...
	SYS=00019999
	${SHELL_DIR}/cardh52.sh -a ${SYS} > ${RPT_DIR}/tst-cardh52 2>&1
	echo "     `grep "WRITTEN" ${RPT_DIR}/tst-cardh52`"
