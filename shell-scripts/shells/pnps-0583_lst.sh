#!/bin/ksh
#
# Program Name	: pnps-0583_lst.sh
# Description	: Creates log listing of PNPS/IDA eligibility file.
...
# 		: 05/03/2018 - Add the ToPDMI to the FTP_DIR Variable. (DME)
#
...
FTP_DIR="/usr/lnk/wt/ida-ftp/ToPDMI"
...
usage: pnps-0583_lst.sh [-d <ccyymmdd>]
...
set_filename()
{
        PGP_FILE="pnpselg${INPUT_DATE}.txt.pgp"
	ELIG_FILE="pnpselg${INPUT_DATE}.txt"
}
...
create_listing()
{
	LOG_NAME=pnps-0583-${DATE}.log
        cd ${ELIG_DIR}
        echo "PNPS-IDA (spo0583) Eligibility File" > ${ELIG_LOG}/${LOG_NAME}
