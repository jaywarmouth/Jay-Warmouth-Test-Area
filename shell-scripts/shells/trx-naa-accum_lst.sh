#!/bin/ksh
#
# Program Name	: trx-naa-accum_lst.sh
# Description	: Creates log listing of TRX-NAA Accumulator file.
...
usage: trx-naa-accum_lst.sh [-d <ccyymmdd>]
...
set_filename()
{
        PGP_FILE="TRX-NAA-Accum-${INPUT_DATE}.txt.pgp"
	ACCUM_FILE="TRX-NAA-Accum-${INPUT_DATE}.txt"
}
...
create_listing()
{
	LOG_NAME=${CLIENT}l-${DATE}.log
        cd ${ELIG_DIR}
        echo "TRX-NAA Accumulator File" > ${ELIG_LOG}/${LOG_NAME}
