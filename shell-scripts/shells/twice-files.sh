...
${SHELL_DIR}/claim111d0.sh -c twice > ${RPT_DIR}/twice-claim111d0 2>&1
${SHELL_DIR}/claim111rx.sh -c twice > ${RPT_DIR}/twice-claim111rx 2>&1
${SHELL_DIR}/clmrt01.sh -c twice -v > ${RPT_DIR}/twice-clmrt01 2>&1

# Convert output files to PDF and email
echo "### twice-claim111d0 ###" > ${RPT_DIR}/twice-twice-files
cat ${RPT_DIR}/twice-claim111d0 >> ${RPT_DIR}/twice-twice-files
echo "### twice-claim111rx ###" >> ${RPT_DIR}/twice-twice-files
cat ${RPT_DIR}/twice-claim111rx >> ${RPT_DIR}/twice-twice-files
echo "### twice-clmrt01 ###" >> ${RPT_DIR}/twice-twice-files
cat ${RPT_DIR}/twice-clmrt01 >> ${RPT_DIR}/twice-twice-files

enscript -Rgj --non-printable-format=space -o - ${RPT_DIR}/twice-twice-files | ps2pdf - ${RPT_DIR}/twice-twice-files.pdf

echo "Output from twice-files.sh process" | ${MAIL_PROG} -s "twice-cycle - twice-files" ${MAIL_TO} -a ${RPT_DIR}/twice-twice-files.pdf 
