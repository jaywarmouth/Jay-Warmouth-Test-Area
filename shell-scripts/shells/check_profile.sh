...
do
	...
	#	echo $profile
                if [ -f "$profile" ]
                then
			cat $profile | grep -v "^#" | egrep "exec /usr/bin/passwd|exec /usr/pdm/bin/login_netw.sh|exec /usr/local/bin/login_netw-2s.sh|exec sleep 60|exec /usr/local/bin/login_passwd.sh" > $TMPFILE
