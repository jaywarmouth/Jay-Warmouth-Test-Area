#!/bin/ksh
#
# Program Name	: schn-basaccum_lst.sh
# Description	: Creates log listing of SCHN-BAS Accumulator file.
...
usage: schn-basaccum_lst.sh [-d <ccyymmdd>]
...
set_filename()
{
        PGP_FILE="SCHNBAS_accum_${INPUT_DATE}.txt.asc"
	ACCUM_FILE="SCHNBAS_accum_${INPUT_DATE}.txt"
}
...
create_listing()
{
	LOG_NAME=${CLIENT}l-${DATE}.log
        cd ${ELIG_DIR}
        echo "SCHN-BAS Accumulator File" > ${ELIG_LOG}/${LOG_NAME}
