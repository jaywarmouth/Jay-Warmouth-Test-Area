#!/bin/ksh
#
# Program Name	: clms_theracom.sh
# Description	: Procedure to provide cycle refresh file for TheraCom (sys0152)
...
ID="THC"
MAIL_PROG="/bin/mail"
MAIL_TO="cambush@thera.com dataerrors@thera.com"
...
usage: clms_theracom.sh -p <p/e date>
...
set_filenames()
{
	...
	CLM_FILE="bimonthlyTHCclms${FILE_DATE}.txt"
	LOG_FILE="bimonthlyTHCtotals${FILE_DATE}.txt"
}
...
ftp_file()
{
	...
	then
	   ...
	   echo "The files for P/E ${PE_DATE} are now available." | ${MAIL_PROG} -s "TheraCom Cycle Refresh File Notification" -c ${MAIL_CC} ${MAIL_TO}
	else
