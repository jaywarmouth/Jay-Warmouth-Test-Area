#!/bin/ksh
#
# Program Name	: ss-mhn_lst.sh
# Description	: Creates log listing of ScriptSense/Montana Health eligibility file.
#                 Command Line Arguments:
#		  -d <ccyymmdd> - date on file sent
# Author	: Linda S. Jefferis
# Date		: 01/02/2006
# Modifications : 08/23/2006 - Changes for 4-digit system number  (LSJ)
#		: 01/19/2011 - Updates for new daily X12 formatted files  (LSJ)
#
# Variables Used:
...
usage: ss-mhn_lst.sh -d <ccyymmdd>
...
set_filename()
{
        PGP_FILE="mhn${INPUT_DATE}.pgp"
	ELIG_FILE="mhn${INPUT_DATE}"
}
...
create_listing()
{
	LOG_NAME=ss-mhn${DATE}.log
        cd ${ELIG_DIR}
        echo "ScriptSense/Montana Health (sys68/spo0451) Eligibility File" > ${ELIG_LOG}/${LOG_NAME}
