...
#		  -m Merge all claims flag (bypasses check if certain claims already exist)
# Author        : Dave Tucci      
# Date          : 04/25/97
# Modifications : 04/29/97 - Added env_var & OBJ_DIR logic  (LSJ)
#		: 10/24/2005 - Changes for Linux  (LSJ)
#		: 12/23/2010 - Added MERGE_ALL 
#
# Variables Used:
...
usage: claim105.sh [-i <filename>] [-f <filename>] [-b <batchrange>] -m
...
then
  ...
  if [ ${ARG_DATES_SW} = 1 ]
  then
    runcobol ${OBJ_DIR}/claim105 -s 1${MERGE_ALL} -a ${ARG_DATES} 
  else
    runcobol ${OBJ_DIR}/claim105 -s 0${MERGE_ALL} 
  fi
fi
