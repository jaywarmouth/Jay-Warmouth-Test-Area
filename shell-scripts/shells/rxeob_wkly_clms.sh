#!/bin/ksh
#
# Program Name	: rxeob_wkly_clms.sh
# Description	: Prepare claims file for FTP to RXEOB
#               : Command Line Arguments:
#                       -d <mmdd> Date for filename
# Author	: Linda Jefferis
# Date		: 01/12/2010
# Modifications : 07/02/2010 - Added "?" to RXEOB file names
#
# Variables Used:
DATE=`date`
FILE_DATE="null"
...
TAPE_FILE=???CL111RX-W-RXEOB			# Original file
LOG_FILE=???-W-RXEOBTEXT			# Summary of totals file
...
usage: rxeob_wkly_clms.sh [-d <mmdd>]
	mmdd - current date
...
set_filenames()
{
   ...
   then
      ...
      ZIP_FILE=wk_${FILE_DATE}.zip      # Zip file name
      CLM_FILE=wk_${FILE_DATE}.txt	# New File name
   fi
}
