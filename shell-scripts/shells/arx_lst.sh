...
REMOTE_DIR="/usr/lnk/wt/arx-ftps/topdmi"
...
set_filename()
{
	ELIG_FILE="ELG${INPUT_DATE}.txt"
	ACC_FILE="ACC${INPUT_DATE}.txt"
	EXC_FILE="EXC${INPUT_DATE}.txt"
}
...
move_files()
{
	...
	### EXC File
	if test -e ${REMOTE_DIR}/${EXC_FILE}
	then
		if test -s ${REMOTE_DIR}/${EXC_FILE}
		then
			EXC=1
			mv ${REMOTE_DIR}/${EXC_FILE} ${ELIG_DIR}/${CLIENT}x${DATE}
			cp ${ELIG_DIR}/${CLIENT}x${DATE} ${ELIG_ARCH}
		else
			mv ${REMOTE_DIR}/${EXC_FILE} ${ELIG_ARCH}/sys${SYS}/${CLIENT}x${DATE}
		fi
	else
...
create_listing()
{
	LOG_NAME=arx-${DATE}.log
        cd ${ELIG_DIR}
        echo "Assist Rx (SYS0123) Files" > ${ELIG_LOG}/${LOG_NAME}
