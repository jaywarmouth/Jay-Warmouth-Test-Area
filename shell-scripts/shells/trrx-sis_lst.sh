#!/bin/sh
#
# Program Name	: trrx-sis_lst.sh
# Description	: Creates log listing of TrueRx-Sisco eligibility file.
...
usage: tsc-umrelig_lst.sh [-d <ccyymmdd>]
...
set_filename()
{
        PGP_FILE="SiscoTRX_elig_${INPUT_DATE}.dat.pgp"
	ELIG_FILE="SiscoTRX_elig_${INPUT_DATE}.dat"
}
...
create_listing()
{
	LOG_NAME=${CLIENT}e-${DATE}.log
        cd ${ELIG_DIR}
        echo "TrueRx-Sisco(SPO1384) Eligibility File" > ${ELIG_LOG}/${LOG_NAME}
