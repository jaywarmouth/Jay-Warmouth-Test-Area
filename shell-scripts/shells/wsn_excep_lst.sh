#!/bin/ksh
#
# Program Name	: wsn_excep_lst.sh
# Description	: Creates log listing of WSN (164) exception file.
...
usage: wsn_excep_lst.sh [-d <ccyymmdd>]
...
set_filename()
{
	ELIG_FILE="WSN_excep_${INPUT_DATE}.txt"
}
...
move_files()
{
	...
	then
		mv ${REMOTE_DIR}/${ELIG_FILE} ${ELIG_DIR}/${CLIENT}x${DATE}
...
create_listing()
{
	LOG_NAME=${CLIENT}x-${DATE}.log
        cd ${ELIG_DIR}
        echo "WSN (sys0164) Exception File" > ${ELIG_LOG}/${LOG_NAME}
