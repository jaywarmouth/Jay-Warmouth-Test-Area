...
{  case ${FILE} in
     ...
     "/usr/local/logs/linedrv/switch60/"  | \
     "/usr/local/logs/linedrv/switch70/"  | \
     "/usr/local/logs/linedrv/switch90/"  | \
     "/usr/local/logs/linedrv/switch_medsub/"  | \
...
{
	sleep 5s
	DATETM=`date +%Y%m%d-%H%M%N`
  	nohup ${SHELL_PATH}/create_rxconnect_record.sh -r ${RUNTYPE} -f ${FILE} > /dev/null 2>&1 & 
...
#=================
##
#rm -f ${SPLIT_FILE_TEMP_LOC}-${RUNTYPE}-UPDT 
